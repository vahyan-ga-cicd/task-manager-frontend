...
import {
  getalldata,
  getPublicStats,
} from "@/utils/api/decentralized/publicapi";
import {
  LineChart,
  Line,
  BarChart,
...
export default function TaskDashboard() {
  ...
  const [lastKeyHistory, setLastKeyHistory] = useState<(string | null)[]>([
    null,
  ]);

  const fetchTasks = useCallback(
    async (targetKey: string | null = null, newPage: number = 1) => {
      try {
        setPageLoading(true);
        const res = await getalldata(10, targetKey);
  ...
  const stats = useMemo(() => {
    ...
    const counts = {
      completed:
        ...
        tasks.reduce(
          (acc, t) =>
            normalizeStatus(t.status) === "completed" ? acc + 1 : acc,
          0,
        ),
      ongoing:
        globalStats?.ongoing ??
        tasks.reduce(
          (acc, t) => (normalizeStatus(t.status) === "ongoing" ? acc + 1 : acc),
          0,
        ),
      pending:
        globalStats?.pending ??
        tasks.reduce(
          (acc, t) => (normalizeStatus(t.status) === "pending" ? acc + 1 : acc),
          0,
        ),
      "on-hold":
        globalStats?.["on-hold"] ??
        tasks.reduce(
          (acc, t) => (normalizeStatus(t.status) === "on-hold" ? acc + 1 : acc),
          0,
        ),
    };

    const totalCount = globalStats?.total ?? tasks.length;
    ...
    const statusData = [
      ...
      },
      {
        ...
        color: STATUS_CONFIG["on-hold"].color,
      },
    ];
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                                ...
                                <div>
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {task.assigned_to_role && (
                                      <span className="px-1 py-0.5 rounded bg-blue-50 text-[8px] font-bold text-blue-500 uppercase leading-none border border-blue-100">
                                        {task.assigned_to_role}
                                      </span>
                                    )}
                                  </div>
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-white flex items-center justify-between flex-wrap gap-3">
                <p className="text-xs text-gray-400 font-medium">
                  ...
                  to{" "}
                  <span className="text-gray-700 font-bold">
                    {Math.min(currentPage * 10, globalStats?.total || 0)}
                  </span>{" "}
                  of{" "}
                  <span className="text-gray-700 font-bold">
                    {globalStats?.total || 0}
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 disabled:opacity-30 disabled:hover:bg-transparent transition-all"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
                  ...
                  {/* Numbered pages */}
                  {(() => {
                    const totalPages = globalStats?.total
                      ? Math.ceil(globalStats.total / 10)
                      : currentPage + (lastKey ? 1 : 0);
                      ...
                      page === "..." ? (
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (page < currentPage) {
                              const targetKey =
                                lastKeyHistory[page - 2] ?? null;
                              fetchTasks(targetKey, page);
                            } else if (page === currentPage + 1 && lastKey) {
                              handleNext();
                            }
                          }}
                          disabled={
                            pageLoading ||
                            page > currentPage + 1 ||
                            (page === currentPage + 1 && !lastKey)
                          }
                          className={`w-8 h-8 rounded-lg text-[11px] font-bold transition-all border
              ${
                page === currentPage
                  ? "bg-gray-900 text-white border-gray-900 shadow-sm"
                  : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50 hover:border-gray-300 disabled:opacity-30 disabled:hover:bg-white"
              }`}
                        >
                          {page}
                        </button>
