...
const AuditLogsTable: React.FC<AuditLogsTableProps> = ({
  ...
  const sanitizeDetails = (text: string) => {
    ...
    if (currentUserName && currentUserEmail)
      identities.push(`${currentUserName} (${currentUserEmail})`);
              ...
              {currentLogs.map((log) => {
                ...
                return (
                  <React.Fragment key={log.log_id}>
                    <tr
                      className={`transition-colors group ${isExpanded ? "bg-indigo-50/30" : "hover:bg-gray-50/60"}`}
                    >
                      {/* Expand toggle */}
                      ...
                      <td className="px-4 py-4 align-middle">
                        <div className="flex items-center gap-2.5">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0 ${isCurrentUser ? "bg-indigo-100 text-indigo-700" : getAvatarColor(log.performed_by_name)}`}
                          >
                            {isCurrentUser
                              ? "You"
                      ...
                      <td className="px-4 py-4 align-middle max-w-[200px]">
                        ...
                        {priorityCfg && (
                          <span
                            className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wide ${priorityCfg.bg} ${priorityCfg.text} ${priorityCfg.border}`}
                          >
                            {log.priority}
                          </span>
                        )}
                      ...
                      <td className="px-4 py-4 align-middle whitespace-nowrap">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border ${actionCfg.bg} ${actionCfg.text} ${actionCfg.border}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${actionCfg.dot}`}
                          ></span>
                          {actionCfg.label}
                        </span>
                      </td>
            ...
            >
              ...
              <div className="px-4 pt-4 pb-3">
                ...
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  <span className="text-[11px] font-mono text-gray-400 tabular-nums flex-shrink-0">
                    {log.timestamp}
                  </span>
                </div>
                ...
                {priorityCfg && (
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wide mb-2 ${priorityCfg.bg} ${priorityCfg.text} ${priorityCfg.border}`}
                  >
                    {log.priority}
                  </span>
                )}
              ...
              <div className="px-4 py-3 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${isCurrentUser ? "bg-indigo-100 text-indigo-700" : getAvatarColor(log.performed_by_name)}`}
                  >
